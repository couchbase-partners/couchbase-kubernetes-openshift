...
class CouchbaseStatefulSet
  ...
  def k8s_template
    {
      ...
      'items' => [
        config_map,
        global_service,
        statefulset_service('query'),
        statefulset('query', 'ephemeral'),
        statefulset_service('data'),
        statefulset('data', nil),
        statefulset_service('index'),
        statefulset('index', nil)
      ]
    }
  end
  ...
  def openshift_template
    {
      ...
      'metadata' =>
      {
        'name' => "couchbase-statefulset-#{storage_type}",
        'annotations' =>
        {
  ...
  def statefulset(role, storage_type)
  ...
  def labels
    { 'template' => "couchbase-statefulset-#{storage_type}-template" }
  end
...
def write_openshift_templates
  ...
  %w(persistent ephemeral).each do |storage_type|
    c = CouchbaseStatefulSet.new('storage_type' => storage_type)
    File.open(dir.join("couchbase-statefulset-openshift-#{storage_type}.yaml"), 'w') do |file|
...
def write_k8s_templates
  ...
  %w(persistent ephemeral).each do |storage_type|
    c = CouchbaseStatefulSet.new(
      'storage_type' => storage_type,
      'image_pull_policy' => 'IfNotPresent',
      'image' => 'couchbase/server:enterprise-4.5.1',
      'param_enabled' => false
    )
    File.open(dir.join("couchbase-statefulset-k8s-#{storage_type}.yaml"), 'w') do |file|
